...
const SUPER_REFUND_PROCESS_API: string =
  "https://vendorgst.fibepe.com/api/User/Vendor/SuperRefundProcess";
  ...
  const handleRecoverTransaction = async () => {
    ...
    try {
      const response = await fetch(
        `${SUPER_REFUND_PROCESS_API}?ledgerID=${ledgerID}`,
        {
          method: "POST",
        }
      );
      ...
      if (data.IsSuccess && data.payLoad) {
        setSuccessMessage(
          `Successfully Refund the amount with the Revert Ledger ID: ${data.payLoad.RevertLedgerId}`
        );
        setIsSuccessModalOpen(true);
        setLedgerID("");
      } else {
        const apiErrorMessage =
          data.payLoad?.StatusMessage ||
          data.StatusCode ||
          "Failed to Refund";
